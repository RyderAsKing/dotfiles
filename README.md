...
- `modes.json` with affordability-based model modes, in cycle order:
  - `economy`: muse-spark-1.3-contributor via opencode-go with xhigh thinking, blue.
  - `balance`: gpt-5.6-luna via openai-codex with max thinking, pink.
  - `premium`: gpt-5.6-terra via openai-codex with high thinking, gold.
...
- An Explore subagent, invoked by Pi through the `subagent` tool, runs in an isolated process with muse-spark-1.3-contributor minimal and read-only tools `read`, `grep`, `find`, `ls`, and `bash`. It returns structured reconnaissance with file ranges, key code, architecture, and a recommended starting point to the primary agent. Subagent result footers also show TPS and TTFT alongside usage.
